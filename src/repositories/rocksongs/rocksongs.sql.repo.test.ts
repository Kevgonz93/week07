...
import { RockSongsSqlRepo } from './rocksongs.sql.repo';
import { type RockSongCreateDto } from '../../entities/rockSongs/rocksong';
...
describe('Given a instance of the class ArticlesSqlRepo', () => {
  const repo = new RockSongsSqlRepo(mockPrisma);

  test('Then it should be instance of the class', () => {
    expect(repo).toBeInstanceOf(RockSongsSqlRepo);
  });

  describe('When we use the method readAll', () => {
    test('Then it should call prisma.findMany', async () => {
      const result = await repo.readAll();
      expect(mockPrisma.rockSong.findMany).toHaveBeenCalled();
  ...
  describe('When we use the method readById with a valid ID', () => {
    test('Then it should call prisma.findUnique', async () => {
      ...
      expect(mockPrisma.rockSong.findUnique).toHaveBeenCalled();
  ...
  describe('When we use the method readById with an invalid ID', () => {
    test('Then it should throw an error', async () => {
      (mockPrisma.rockSong.findUnique as jest.Mock).mockResolvedValueOnce(null);
  ...
  describe('When we use the method create', () => {
    test('Then it should call prisma.create', async () => {
      const data = {} as unknown as RockSongCreateDto;
      const result = await repo.create(data);
      expect(mockPrisma.rockSong.create).toHaveBeenCalled();
      expect(result).toEqual({});
    });
  });
  ...
  describe('When we use the method update with a valid ID', () => {
    test('Then it should call prisma.update', async () => {
      ...
      expect(mockPrisma.rockSong.update).toHaveBeenCalled();
      expect(result).toEqual({});
    });
  });
  ...
  describe('When we use the method delete with a valid ID', () => {
    test('Then it should call prisma.delete', async () => {
      ...
      expect(mockPrisma.rockSong.delete).toHaveBeenCalled();
      expect(result).toEqual({});
    });
  });
